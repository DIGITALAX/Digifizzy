...
const designerCircle = {
  ...
  move(width, height, list) {
    ...
    if (this.x + bubbleSize / 2 < 0) {
      this.x += (width * 0.8) * ((list.length / countPerCol) | 0)
    }
    ...
    if ((newY < bubbleSize / 2 && this.y >= bubbleSize / 2)
      || (newY > height - bubbleSize / 2 && this.y <= height - bubbleSize / 2)) this.vy *= -1
...
const DesignerBubble = () => {
  ...
  const [designerCircles, setDesignerCircles] = useState(Array(designers.length).fill().map((item) => ({ ...designerCircle })))
  const canvasRef = useRef()
  const raf = useRef()

  useEffect(() => {
    const bubbleImage = new Image()
    const photoImages = Array(designers.length).fill().map((item) => new Image())

    Promise.all(designers.map((designer) => fetch(`https://digitalax.mypinata.cloud/ipfs/${designer.CID}`)
      .then((response) => response.json()))).then((designerInfos) => {
