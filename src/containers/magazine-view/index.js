...
const MagazineViewer = forwardRef((props, refs) => {
  ...
  const getChildrenList = () => {
    ...
    pageList.forEach((item, index) => {
      if (((realPageNum > currentIssue.freePageCount && !contentUnlocked) || (realPageNum > ((currentIssue.freePageCount + currentIssue.hiddenPageCount)) && contentUnlocked)) && index < pageList.length - 1) return
      childrenList.push(
