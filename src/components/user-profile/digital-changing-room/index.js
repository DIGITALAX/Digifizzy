...
const DigitalChangingRoom = (props) => {
  ...
  useEffect(() => {
    const fetchDigitalaxSubscriptions = async () => {
      ...
      setOwnedNFTs({
        ...ownedNFTs,
        [DIGIFIZZY_BUNDLES]: {
          ...digitalaxSubscriptionsPolygon.map((item) => ({ ...item, type: 'digitalaxSubscriptionsPolygon' })),
          ...digitalaxSubscription1155sPolygon.map((item) => ({ ...item, type: 'digitalaxSubscription1155sPolygon' }))
        }
      })
    }
  ...
  useEffect(() => {
    const getAllNFTs = async () => {
      ...
      const guildWhitelistedStakedNFTsPolygon = guildWhitelistedNFTStakersPolygon && guildWhitelistedNFTStakersPolygon.length > 0
      ...
      const digitalaxNFTV2sPolygonDrip = [].concat.apply([],
        ...
        ).map((item) => item.garments.map((garment) => ({
          ...garment,
      ...
      const { digitalaxCollectorV2: digitalaxCollectorsV2Polygon } = await getDigitalaxCollectorV2ByOwner(POLYGON_CHAINID, owner)
      ...
      fetchedNFTs[DIGITAL_CHANGING_ROOM] = [
        ...digitalaxNFTsMainnet.map((item) => ({ ...item, type: 'digitalaxNFTsMainnet' })),
        ...digitalaxNFTsPolygon.map((item) => ({ ...item, type: 'digitalaxNFTsPolygon' })),
        ...digitalaxNFTV2sPolygonNonDrip.map((item) => ({ ...item, type: 'digitalaxNFTV2sPolygon' })),
        ...digitalaxNFTV2sPolygonDrip.map((item) => ({ ...item, type: 'digitalaxDripNFTV2sPolygon' })),
        ...weirdItems.map((item) => ({ ...item, type: 'digitalaxNFTV2sPolygon' })),
        ...digitalaxV21155sPolygon.map((item) => ({ ...item, type: 'digitalaxV21155sPolygon' })),
        ...digitalaxStakedNFTsMainnet.map((item) => ({ ...item, type: 'digitalaxStakedNFTsMainnet' }))
      ]
      ...
      fetchedNFTs[GENESIS_MONA_NFT] = [
        ...digitalaxGenesisNFTsMainnet.map((item) => ({ ...item, type: 'digitalaxGenesisNFTsMainnet' })),
        ...digitalaxStakedGenesisNFTsMainnet.map((item) => ({ ...item, type: 'digitalaxStakedGenesisNFTsMainnet' }))
      ]
      ...
      fetchedNFTs[LOOK_FASHION_LOOT] = [
        ...
        ...digitalaxLookGoldenTicketsPolygon.map((item) => ({ ...item, type: 'digitalaxLookGoldenTicketsPolygon' }))
      ]

      fetchedNFTs[PODE] = [
        ...podeNFTv2sPolygon.map((item) => ({ ...item, type: 'podeNFTv2sPolygon' })),
        ...podeStakedNFTsPolygon.map((item) => ({ ...item, type: 'podeStakedNFTsPolygon' }))
      ]

      fetchedNFTs[GDN_MEMBERSHIP_NFT] = [...gdnMembershipNFTsPolygon.map((item) => ({ ...item, type: 'gdnMembershipNFTsPolygon' }))]
    ...
    if (type == 'digitalaxNFTV2sPolygon' || type == 'digitalaxStakedNFTsPolygon') {
      ...
      for (let i = 0; i < subscriptionDetails.length; i += 1) {
        subscriptionDetails[i].forEach((issue) => {
          if (issue.subTitle) {
            const filteredIssues = issue.subTitle.filter((subtitle) => item.name.toLowerCase().includes(subtitle.toLowerCase()))

            if (filteredIssues.length) {
              sub = issue
            }
          } else if (item.name.toLowerCase().includes(issue.title.toLowerCase())) {
            sub = issue
          }
        })
        if (sub) {
